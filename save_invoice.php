...
$date1 = date("YMd");
  ...
  if(!file_exists("/Applications/XAMPP/xamppfiles/htdocs/newinvoice")){

    mkdir("/Applications/XAMPP/xamppfiles/htdocs/newinvoice");


  }
  if(!file_exists("/Applications/XAMPP/xamppfiles/htdocs/newinvoice/$date1")){

    mkdir("/Applications/XAMPP/xamppfiles/htdocs/newinvoice/$date1");
...
if(isset($_POST['submit'])){
  ...
  $medicines = implode(", ", $medicine_name); // Corrected implode usage
  $quantity = $_POST['qty'];
  $qty = implode(", ", $quantity); // Corrected implode usage
  $qty_type = $qty; // This seems redundant, not sure what it's intended for
  $filename = "i-" . $invoice_number . ".pdf";
  $dir = ""; // You haven't defined the directory

  $pdf = new myPDF();
 $pdf->AddPage('L','A4',0);
 $pdf->headerTable();
 $pdf->viewTable();
 $pdf->Output('/Applications/XAMPP/xamppfiles/htdocs/newinvoice/'.$date1.'/'.$filename,'F');
 $pdf->Output('/Applications/XAMPP/xamppfiles/htdocs/newinvoice/'.$filename,'F');
 ...
 $select_on_hold_query = mysqli_query($con,$select_on_hold);
  ...
  $insert_sql = "INSERT INTO sales (invoice_number, medicines, quantity, total_amount, total_profit, date)
